...
#
# EXAMPLES (using this project's own sub-project, baseball_card)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh baseball_card                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev baseball_card     compile just that one, as dev
...
SQRL_VERSION="${SQRL_VERSION:-0.11.4}"
...
suite() {
  case "$1/$2" in
    baseball_card/test) echo "baseball_card-shared-package.json baseball_card-test-package.json" ;;
    baseball_card/dev)  echo "baseball_card-shared-package.json baseball_card-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
