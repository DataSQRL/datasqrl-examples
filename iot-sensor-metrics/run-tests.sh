...
#
# EXAMPLES (using this project's own sub-project, sensors)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh sensors                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev sensors     compile just that one, as dev
...
suite() {
  case "$1/$2" in
    sensors/test) echo "sensors-shared-package.json sensors-test-package.json" ;;
    sensors/dev)  echo "sensors-shared-package.json sensors-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
