...
#
# EXAMPLES (using this project's own sub-project, distressed)
#   ./run-tests.sh                                     test every suite, env test
#   ./run-tests.sh distressed                          test just that sub-project
#   ./run-tests.sh --compile --env test                compile every suite as test
#   ./run-tests.sh --list-invocations --compile        show what would run
#
...
P="${SQRL_PACKAGE_PREFIX:-distressed_loan_dashboard/}"
if [ -n "$P" ]; then
...
# resolves `-b distressed` to <project-root>/build/distressed
...
# project actually has, and leave out the ones it does not.
#
#   <sub-project>/<env>) echo "<base config> <env config>" ;;
#
suite() {
  case "$1/$2" in
    distressed/test) echo "distressed-shared-package.json distressed-test-package.json" ;;
    *) return 1 ;;
  esac
}
