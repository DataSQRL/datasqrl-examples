...
#
# EXAMPLES (using this project's own sub-project, event_audit)
#   ./run-tests.sh                                     test every suite, env test
#   ./run-tests.sh event_audit                         test just that sub-project
#   ./run-tests.sh --compile --env local               compile every suite as local
#   ./run-tests.sh --compile --env prod event_audit    compile just that one, as prod
#   ./run-tests.sh --list-invocations --compile --env local   show what would run
#
...
# This repository is not a git checkout, so the git-based prefix detection below never fires; the
# project's fixed location (a subdirectory of the /workspace mount, alongside the read-only
# data-catalog sibling) is supplied directly as the default so a bare `./run-tests.sh` still
# resolves `-r` correctly. The orchestrator's own SQRL_PACKAGE_PREFIX, if set, still wins.
: "${SQRL_PACKAGE_PREFIX:=distressed_loan_dashboard}"
...
SUBPROJECTS="distress_dashboard"
# ─────────────────────────────────────────────────────────────────────────────
...
suite() {
  case "$1/$2" in
    distress_dashboard/test)  echo "distress_dashboard-shared-package.json distress_dashboard-test-package.json"  ;;
    distress_dashboard/local) echo "distress_dashboard-shared-package.json distress_dashboard-local-package.json" ;;
    distress_dashboard/prod)  echo "distress_dashboard-shared-package.json distress_dashboard-prod-package.json"  ;;
    *) return 1 ;;
  esac
}
