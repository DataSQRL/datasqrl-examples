...
#
# EXAMPLES (using this project's own sub-project, study_analytics)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh study_analytics                         test just that sub-project
#   ./run-tests.sh --compile --env prod                         compile every suite as prod
#   ./run-tests.sh --compile --env prod study_analytics    compile just that one, as prod
...
suite() {
  case "$1/$2" in
    study_analytics/test) echo "study_analytics-shared-package.json study_analytics-test-package.json" ;;
    study_analytics/prod) echo "study_analytics-shared-package.json study_analytics-prod-package.json" ;;
    *) return 1 ;;
  esac
}
