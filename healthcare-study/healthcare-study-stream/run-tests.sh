...
#
# EXAMPLES (using this project's own sub-project, study_stream)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh study_stream                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev study_stream     compile just that one, as dev
...
suite() {
  case "$1/$2" in
    study_stream/test) echo "study_stream-shared-package.json study_stream-test-package.json" ;;
    study_stream/dev)  echo "study_stream-shared-package.json study_stream-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
