...
#
# EXAMPLES (using this project's own sub-project, study_api)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh study_api                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev study_api     compile just that one, as dev
...
suite() {
  case "$1/$2" in
    study_api/test) echo "study_api-shared-package.json study_api-test-package.json" ;;
    study_api/dev)  echo "study_api-shared-package.json study_api-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
