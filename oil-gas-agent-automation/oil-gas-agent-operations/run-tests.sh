...
#
# EXAMPLES (using this project's own sub-project, operations_agent)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh operations_agent                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev operations_agent     compile just that one, as dev
...
suite() {
  case "$1/$2" in
    operations_agent/test) echo "operations_agent-shared-package.json operations_agent-test-package.json" ;;
    operations_agent/dev)  echo "operations_agent-shared-package.json operations_agent-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
