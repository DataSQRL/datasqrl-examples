...
#
# EXAMPLES (using this project's own sub-project, monitoring_agent)
#   ./run-tests.sh                                              test every suite, env test
#   ./run-tests.sh monitoring_agent                         test just that sub-project
#   ./run-tests.sh --compile --env dev                          compile every suite as dev
#   ./run-tests.sh --compile --env dev monitoring_agent     compile just that one, as dev
...
suite() {
  case "$1/$2" in
    monitoring_agent/test) echo "monitoring_agent-shared-package.json monitoring_agent-test-package.json" ;;
    monitoring_agent/dev)  echo "monitoring_agent-shared-package.json monitoring_agent-dev-package.json"  ;;
    *) return 1 ;;
  esac
}
